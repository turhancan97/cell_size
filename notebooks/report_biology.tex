...
automatic quality check (with AI model) and were labelled \textbf{good}
for analysis. Cells labelled bad or rejected were not included in the
size summaries.
...
\textbf{ResNet18} (our AI model) classifier, which outputs
\textbf{p(good)} --- the model's estimated probability that the cell
quality is good enough to measure. We used \textbf{selective rejection}
with three outcomes:
...
\item
  ...
  (\(\mu\)m)
\item
  \textbf{Nucleus area and long / short diameter}, \textbf{N/C ratio}
  (nucleus area \(\div\) cell area), and \textbf{axis ratio} (long
  \(\div\) short axis)
\end{itemize}
\end{keystats}
...
\item
  ...
  \(\mu\)m).
\item
  \textbf{Axis ratio (major \(\div\) minor): median 1.62} --- values
  above 1 mean the cell is elongated.
\end{itemize}
\end{keystats}
...
\item
  ...
  \(\mu\)m\(^2\)); \textbf{57,218} good cells with a matched nucleus
  mask.
\item
  \textbf{Nucleus long axis: 8.96 \(\mu\)m}; \textbf{short axis: 5.66
  \(\mu\)m} (medians across matched cells).
\end{itemize}
\end{keystats}
...
The table above summarises \textbf{central 95\% reference intervals}
(2.5th--97.5th percentile) for good cells in this cohort. \textbf{Cell
area} spans \textbf{200.90--463.10 \(\mu\)m\(^2\)} (median
\textbf{290.69 \(\mu\)m\(^2\)}). Among cells with a matched nucleus,
\textbf{nucleus area} falls between \textbf{28.57} and \textbf{63.36
\(\mu\)m\(^2\)} (median \textbf{39.06 \(\mu\)m\(^2\)}), and \textbf{N/C
ratio} between \textbf{0.09} and \textbf{0.21} (median \textbf{0.13}).
